...
#
# Usage:
#   .\scripts\script-release.ps1 -Version 1.0.0
#   .\scripts\script-release.ps1 -Version 1.0.0 -DryRun
#   .\scripts\script-release.ps1 -Version 1.0.0 -ForceBuild
# =============================================================================
...
if (-not (Test-Path "NvidiaCi.csproj")) {
    Write-Host "  [FAIL] Jalankan script ini dari root project (tempat NvidiaCi.csproj berada)." -ForegroundColor Red; exit 1
}
...
$files = @(
    ...
    @{
        ...
        Replace = "<AssemblyVersion>$Version.0</AssemblyVersion>"
    }
)
...
foreach ($f in $files) {
    ...
    # Inject jika tag belum ada
    if ($content -eq $newContent -and $f.Replace -notmatch $content) {
        $newContent = $content -replace '</PropertyGroup>', "    $($f.Replace)`n  </PropertyGroup>"
    }
